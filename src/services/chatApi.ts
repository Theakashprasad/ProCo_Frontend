...
export const getChatorCreateChat = async (currentUserId: string, otherUserId: string) => {
  ...
  const response = await axios.get(`${BACKEND_URL}/api/chat/${currentUserId}/${otherUserId}`)
  return response.data
}

export const getChatMessages = async (chatId: string) => {
  console.log(chatId, 'id')
  const response = await axios.get(`${BACKEND_URL}/api/chat/message/${chatId}/messages`)
  return response.data
}
...
export const getUserChats = async (userId: string) =>{  
  const response = await axios.get(`${BACKEND_URL}/api/chat/chats/${userId}/getuserchats`)
