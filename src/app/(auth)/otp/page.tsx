...
const Otp = () => {
  ...
  // Fetch stored email from localStorage when the component mounts
  useEffect(() => {
    const signup = localStorage.getItem("userEmailFromSignUpPage");
    const forget = localStorage.getItem("userEmailFromForgetPage");
    setStoredEmail(signup || forget || ""); // Set the stored email
    if (signup) setNextUrl(true);
    else setNextUrl(false);
  }, []);
  ...
  const resentHandler = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || "Resending OTP failed", {
          position: "top-center",
        });
      }
    } catch (error) {
  ...
  return (
    <div className="bg-black flex items-center justify-center min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center w-full max-w-screen-lg ml-14 md:px-0">
        <div className="md:w-1/2 text-center md:text-left mb-8 md:mr-40">
        ...
        <div className="md:w-1/3 bg-black text-white p-6 md:p-10 rounded-lg shadow-lg max-w-md h-[30rem] relative backdrop-blur-md bg-transparent border border-purple-600">
          ...
          <button type="button" onClick={resentHandler}>
              Resent
            </button>
